...
class variables():
 running=0
 map=[-1 for _ in range(16*16)]
 buffer=[]
 help=["commands+AliAses:", "'Help' 'ColoR # # #'('zx') 'EXport (name)'", "'EYEdropper' 'Cut' 'Paste' 'BackGround'", "'IMport (name)'", "controls:","enter backspace mouse space", "'LMB' 'RMB' 'wasd' 'zx(history)'", "'frgthy(color)(shift)' 'cv(cutPaste)'", "'c(shift)'"]
 console=""
 loc=0
 ...
 background= -1
v= variables()
#funcs: visibleCursor() pickColor(r,g,b,set) hexColor(c, set) disableControls() hexToHue() draw() graphics() historyFunc(set) commands(force, mod) backSpace(key) onPress(key)
...
def draw():
 ...
 if v.consoleState==1:
  ...
  print(f"\033[{v.loc//16};{v.loc%16+1}H", end="")
...
def graphics():
 ...
 if v.consoleState== 6:
  for i in range(len(v.copyMap)):
   if v.copyMap[i][1]!=-1:
    newMap[max(min(16*16-1, v.copyMap[i][0]+v.loc), 0)]= v.copyMap[i][1]
 for y in range(v.xy[1]-1):
  ...
  for x in range(v.xy[0]-1):
   if v.consoleState==5:
    ...
    v.buffer[y]+=' '
    continue
   if y*16+x== v.loc and x<16 and y<16 and not v.consoleState in [1,7]:
    ...
    if y*16+ x in v.saveSelect and y<16 and x<16:
     v.buffer[y]+= pickColor(v.selectColor)+ selectChar+ reset
     continue
   newCol= ''
   if x<16 and y<16:
    if newMap[y*16+x]!=-1:
     newCol= pickColor(newMap[y*16+x])
     v.buffer[y]+= newCol+ ' '+ reset
     continue    
   borderChar=' '
   if x==16 and y<16: borderChar= '|'
   elif y==16 and x<16: borderChar= '-'
   colorView= ''
   if x<16 and y<16: colorView= pickColor(v.background)
   if x>18 and x<25 and y>0 and y<4:
...
def commands(force= 0, mod=0): #0-None 1-cut 2-paste
 ...
 if c.startswith("export") or c.startswith("ex"):
  ...
  if v.consoleState!= 6:
   ...
   v.consoleState= 0
   for i in range(len(v.copyMap)):
    v.map[max(0,min(16*16,v.copyMap[i][0]+v.loc))]= v.copyMap[i][1]
...
def onPress(key):
  ...
  if key == '\x1b':  # Escape
      ...
      try:
       ...
       if len(parts) >= 3:
        ...
        if seq[-1] == 'M':  # Mouse button pressed
         if mouse_x < 16 and mouse_y < 16:
          v.loc = mouse_y*16+mouse_x
          if button in [0,2]:
           if v.consoleState!=7: 
            historyFunc(0)
            v.consoleState=0
          if button in [0,32]: 
            v.map[mouse_y*16+mouse_x] = v.selectColor
          elif button in [2,34]: 
           v.map[mouse_y*16+mouse_x] = -1
   ...
   if v.consoleState==1: v.console += key
    ...
    if key.lower() == 'a':
     if v.loc%16>0: v.loc= v.loc-1
    elif key.lower() == 'd':
     if v.loc%16<15: v.loc= v.loc+1
    elif key.lower() == 'w':
     if v.loc//16>0: v.loc= v.loc-16
    elif key.lower() == 's':
     if v.loc//16<15: v.loc= v.loc+16
    if key.lower() == 'z':
    ...
    if key.lower() in ['a','w','s','d']:
     if v.consoleState in [2,3,4]:
      v.saveSelect = [y* 16+ x for y in range(min(v.selectLoc//16, v.loc//16), max(v.selectLoc//16, v.loc//16) + 1)
      for x in range(min(v.selectLoc%16, v.loc%16), max(v.selectLoc%16, v.loc%16) + 1)]
...
while v.running!=2:
 try:
  ...
  print( f"\033]0;{"PAUSED" if v.running==1 else ""}{newDebug}{intToRgb}/paint\007", end='', flush=True)
